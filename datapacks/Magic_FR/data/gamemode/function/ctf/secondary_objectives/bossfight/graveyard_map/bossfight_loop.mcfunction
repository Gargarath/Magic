...
execute as @e[type=wither_skeleton,tag=lich] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/display_bossbar
...
execute if score invulnerable lich matches 0 unless entity @a[tag=targetable] as @e[type=minecraft:wither_skeleton,tag=lich,limit=1] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/invulnerable/become_invulnerable

# rend la liche vulnérable quand il y a des joueurs dans l'église
execute if score invulnerable lich matches 1 if entity @a[tag=targetable] as @e[type=wither_skeleton,tag=lich,limit=1] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/invulnerable/no_more_invulnerability
execute if score invulnerable lich matches 1 at @e[type=wither_skeleton,tag=lich] if entity @a[tag=invisibility_r,distance=..5] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/invulnerable/no_more_invulnerability

# gere les cd de la liche
execute if score cooldown lich matches 1.. run scoreboard players remove cooldown lich 1
execute if score cooldown lich matches 0 run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/spells/cooldown_end
...
execute if score cast_time lich matches 0 run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/spells/launch_spell
# réduit le cast time -> quand il arrive a 0 lance le spell

# gere la mort de la liche
execute unless entity @e[type=wither_skeleton,tag=lich] if score is_dead lich matches 0 run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/lich_death

# gere les lancements de spells
execute if score is_using_spell lich matches 0 unless score cooldown lich matches 0.. if entity @a[tag=targetable] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/spells/cast_spell
...
execute as @e[type=marker,tag=lich_aoe] at @s run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/spells/spell3/update_aoe
# gère l'aoe 1 de la liche (dégats / particule / vie)


# gere la victoire d'une des deux équipes
execute as @a[scores={Player=1..},nbt={Inventory:[{id:"minecraft:nether_star"}]}] run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/picked_up_item

execute as @a[scores={Player=1..}] at @s run function gamemode:ctf/secondary_objectives/bossfight/graveyard_map/determine_targetable
