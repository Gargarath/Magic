# appelée par ctf:respawns/respawn_has_flag ou ctf:respawns/respawn permet de cacluculer combien de temps @s doit prendre pour respawn et d'en ajouter à l'équipe rouge.
...
execute store result score @s ppl_dead_in_team if entity @a[team=respawn_red]
...
execute if score @s ppl_dead_in_team matches 1.. run scoreboard players add @a[team=respawn_red,scores={respawn_time=0..}] max_respawn_bar 40
...
scoreboard players set @a[team=respawn_red,scores={max_respawn_bar=241..}] max_respawn_bar 240
execute if score $teamkill option_panel matches 1 if score red playercount matches 3.. run execute unless entity @a[team=red] run function gamemode:ctf/respawns/bossbar/teamkill/teamkill_red
