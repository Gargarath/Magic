...
execute unless entity @a[tag=target,nbt={active_effects:[{id:"minecraft:blindness"}]}] run function ctf:secondary_objectives/bossfight/graveyard_map/spells/spell_randomizer/with_blind
# si la target n'a pas l'effet n°15 (blindness) -> lance le randomizer classique
execute if entity @a[tag=target,nbt={active_effects:[{id:"minecraft:blindness"}]}] run function ctf:secondary_objectives/bossfight/graveyard_map/spells/spell_randomizer/without_blind
# si la target a pas effet n°15 (blindness) -> lance le randomizer sans le sort qui ne donne pas blindness

schedule function ctf:secondary_objectives/bossfight/graveyard_map/spells/set_spell_cast_time/set_cast_time 1.5s replace