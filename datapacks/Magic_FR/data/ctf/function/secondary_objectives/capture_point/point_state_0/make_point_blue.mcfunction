...
bossbar set minecraft:secondary_objective_blue color blue
bossbar set minecraft:secondary_objective_red color blue
scoreboard players set state point_capture 1
# passe au state 1 (= les bleus contrôlent le point)

tellraw @a {"text":"L'équipe bleue contrôle l'objectif secondaire !","color":"blue"}
...
execute store result bossbar minecraft:secondary_objective_blue max run scoreboard players get max_blue point_capture
execute store result bossbar minecraft:secondary_objective_red max run scoreboard players get max_blue point_capture
