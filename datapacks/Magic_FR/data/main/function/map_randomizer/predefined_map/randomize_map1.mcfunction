## appelée predefined_map si on empêche de jouer deux fois la même map et que la map 1 est en aléatoire
## permet de sélectionner la première map au hazard sans que ce soit comme les suivantes et de l'enregistrer
...
execute if score %rng_map map_selection = fourth_map map_memories run scoreboard players set $already_taken map_selection 1
# vérifie que la map actuelle n'est pas indentique à la map 4
...
execute if score $already_taken map_selection matches 0 run scoreboard players operation first_map map_memories = selected_map variables
# si pas d'erreur -> enregistre la map

execute if score $already_taken map_selection matches 1 run function main:map_randomizer/predefined_map/randomize_map1
