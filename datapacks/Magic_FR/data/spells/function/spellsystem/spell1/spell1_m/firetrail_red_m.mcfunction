...
execute if block ~ ~ ~ #spell_beam unless block ~ ~-45 ~ #spawn_protectors positioned ~ ~-1 ~ unless entity @e[type=#boss,distance=..0.9,tag=!spell_immune] unless entity @a[distance=..0.9,tag=!spell_immune,team=blue] unless entity @a[distance=..0.9,tag=!spell_immune,team=red] positioned ^ ^ ^1 positioned ~ ~1 ~ run function spells:spellsystem/spell1/spell1_m/firetrail_red_m
