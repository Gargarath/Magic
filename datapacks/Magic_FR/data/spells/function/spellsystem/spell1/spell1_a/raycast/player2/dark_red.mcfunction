...
execute as @e[type=#spellable,distance=..1,team=!spectator] run tag @s add in_explo2
execute if score @s explo_position_a < @a[scores={Player=2},limit=1] explo_max_range_a positioned ^ ^ ^0.5 if block ~ ~ ~ #traversable_blocs run function spells:spellsystem/spell1/spell1_a/raycast/player2/dark_red