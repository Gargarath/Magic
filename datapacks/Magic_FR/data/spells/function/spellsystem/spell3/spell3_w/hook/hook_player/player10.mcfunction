## called by extand_hook/rotate_hook/player10_blue if it collides with a player
## makes the player hooked 

function spells:spellsystem/spell3/spell3_w/hook/retract_hook/start_delete/player10
# launch the retracting of the hook

tag @s remove has_hooked
# indicate that the launcher has hooked someone

execute if entity @a[scores={Player=10},team=blue,tag=!no_team] run scoreboard players set @r[team=red,distance=..1] hooked_by_player 10
execute if entity @a[scores={Player=10},team=red,tag=!no_team] run scoreboard players set @r[team=blue,distance=..1] hooked_by_player 10
execute if entity @a[scores={Player=10},tag=no_team] run scoreboard players set @r[tag=!hook10_immune,distance=..1] hooked_by_player 10
# indicate that the hooked player has been hooked by player 10 depending on his team

execute as @a[scores={hooked_by_player=10}] run function spells:spellsystem/spell3/spell3_w/hook/hook_player/effect_hooked_player
...
execute as @a[scores={Player=10}] at @s run playsound minecraft:entity.evoker.cast_spell master @s ~ ~ ~ 100 2 1
