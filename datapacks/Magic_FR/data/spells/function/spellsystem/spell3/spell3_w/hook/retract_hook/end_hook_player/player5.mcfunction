## called by retract_hook/delete_hook/player5 if there is no more pieces of the hook
...
execute unless score @s IsAlive matches 1 unless score @s IsAlive matches 501 at @s[team=!respawn_blue,team=!respawn_red] positioned ^ ^ ^1.2 run tp @a[scores={IsAlive=0,hooked_by_player=5}] ~ ~0.8 ~
execute unless score @s IsAlive matches 1 unless score @s IsAlive matches 501 at @s[team=!respawn_blue,team=!respawn_red] positioned ^ ^ ^1.2 run tp @a[scores={IsAlive=500,hooked_by_player=5}] ~ ~0.8 ~
# tp the hooked player in front of @s

execute unless score @s IsAlive matches 1 unless score @s IsAlive matches 501 as @a[scores={IsAlive=0,hooked_by_player=5}] at @s facing entity @a[scores={Player=5}] eyes run tp @s ~ ~ ~ ~ ~40
execute unless score @s IsAlive matches 1 unless score @s IsAlive matches 501 as @a[scores={IsAlive=500,hooked_by_player=5}] at @s facing entity @a[scores={Player=5}] eyes run tp @s ~ ~ ~ ~ ~40
# makes the hooked player look at @s

scoreboard players set @a[scores={IsAlive=0,hooked_by_player=5}] last_caster 5
scoreboard players set @a[scores={IsAlive=500,hooked_by_player=5}] last_caster 5
# si la cible est encore en vie -> tag pour check après 1 tick les pv des joueurs - le tag dépend du numéro de joueur du caster

execute as @a[scores={IsAlive=0,hooked_by_player=5}] run function spells:spellsystem/spell3/spell3_w/hook/apply_dmg
execute as @a[scores={IsAlive=500,hooked_by_player=5}] run function spells:spellsystem/spell3/spell3_w/hook/apply_dmg
