...
execute as @a[scores={hooked_by_player=8}] run function spells:spellsystem/spell3/spell3_w/hook/hook_player/set_bossbar/player8
# display the bossbar to the player that has been hooked depending on his player level

bossbar set hook_someone:player8 name ["",{"text":"Vous avez agrippé ","color":"gray","bold":true},{"selector":"@a[scores={hooked_by_player=8}]","bold":true}]
