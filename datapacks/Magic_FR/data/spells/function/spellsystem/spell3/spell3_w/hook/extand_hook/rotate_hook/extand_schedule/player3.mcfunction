...
execute unless score @a[scores={Player=3},limit=1] hook_max_range_w matches 1 at @s positioned ~ ~0.5 ~ unless block ~ ~-45 ~ #spawn_protectors if entity @a[tag=!hook3_immune,distance=..1] as @a[scores={Player=3},limit=1] run function spells:spellsystem/spell3/spell3_w/hook/hook_player/player3 with entity @s EnderItems[0].components.minecraft:custom_data
