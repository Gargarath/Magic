...
execute store result entity @s Pose.Head[0] float 1 run data get entity @a[scores={Player=8},limit=1] Rotation[1]
# turn @s head inclinasion to the same as the player

tag @s remove hook_w_player8_new
...
schedule function spells:spellsystem/spell3/spell3_w/hook/extand_hook/rotate_hook/extand_schedule/player8_schedule 1t
