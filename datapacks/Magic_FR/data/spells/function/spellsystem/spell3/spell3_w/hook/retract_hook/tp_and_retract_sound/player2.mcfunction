# called by delete_hook_player2
# Tp the hooked player to the head piece of the hook plays a sounds and delete the head piece of the hook

execute at @s run tp @a[scores={IsAlive=0,hooked_by_player=2}] ~ ~0.8 ~
execute at @s run tp @a[scores={IsAlive=500,hooked_by_player=2}] ~ ~0.8 ~
