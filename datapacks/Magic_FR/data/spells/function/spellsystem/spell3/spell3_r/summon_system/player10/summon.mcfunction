...
execute if entity @a[scores={Player=10},tag=no_team] at @e[type=area_effect_cloud,tag=smoke_bomb_position10] run summon area_effect_cloud ~ ~ ~ {custom_particle:{type:"block",block_state:"minecraft:air"},Duration:160,Tags:["smoke_bomb","smoke_bomb_lvl1","smoke_bomb_player10"]}
...
$bossbar set smoke_bomb:player10 name {"text":"$(tr_rogue_spell3_1_name) :","color":"gray","bold":true}
