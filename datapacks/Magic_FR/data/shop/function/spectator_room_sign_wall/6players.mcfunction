# appelée par shop:shop_is_ready lorsqu'il y a 6 joueurs dans la partie, permet alors d'afficher les panneaux pour les observer
...
$setblock 0 -49 61 oak_wall_sign[facing=north]{front_text:{messages:[{"text":"$(tr_shop_spec)","color":"white","bold":true,"click_event":{"action":"run_command","command":"trigger spec_player6 add 1"}},"",{"nbt":"by_player.6.name","storage":"stats:leaderboards","interpret":true},""]},is_waxed:1b} replace
