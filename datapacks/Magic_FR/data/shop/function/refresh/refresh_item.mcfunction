...
$execute unless score %-1 numbers matches $(price) if score @s PH matches $(price).. run data merge entity @n[type=text_display,tag=shop_$(item_category)_price,tag=shop_$(item_category)_$(player)] {text:{"color":"green","text":"$(price)"}}
# rend le prix rouge si trop cher
$execute unless score %-1 numbers matches $(price) unless score @s PH matches $(price).. run data merge entity @n[type=text_display,tag=shop_$(item_category)_price,tag=shop_$(item_category)_$(player)] {text:{"color":"red","text":"$(price)"}}
