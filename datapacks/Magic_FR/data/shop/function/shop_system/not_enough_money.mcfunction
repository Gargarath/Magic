## appelée par test quand @s n'a pas assez d'argent
# lui dit qu'il ne peut pas acheter

execute at @s run playsound minecraft:block.note_block.bass master @s ~ ~ ~ 100 0

$data modify storage gui player.$(player).actionbar.alert set value {"text":"$(tr_shop_item_not_enough_money) ","color":"dark_red","bold":false}
