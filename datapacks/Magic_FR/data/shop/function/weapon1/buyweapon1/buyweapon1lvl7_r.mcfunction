# Permet d'acheter la weapon1 de niveau 7 pour l'assassin
...
advancement grant @s only rogue:weapon1lvl7_r
# Donne le haut fait "Dague sournoise VII" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:carrot_on_a_stick",count:1,components:{"minecraft:custom_name":"Dague sournoise VIII - 80 PH","custom_model_data":{strings:["dagger"]}}},ItemDropChance:1.0f}
