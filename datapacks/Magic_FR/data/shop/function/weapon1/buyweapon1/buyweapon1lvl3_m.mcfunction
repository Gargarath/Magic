# Permet d'acheter la weapon1 de niveau 3 pour le mage
...
advancement grant @s only mage:weapon1lvl3_m
# Donne le haut fait "Bâton de Mage III" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:stick",count:1,components:{"minecraft:custom_name":"Bâton de Mage IV - 40 PH"}},ItemDropChance:1.0f}
## Remplace l'objet weapon1 par un objet de niveau superieur et le remet à la position 0 dans l'item frame

scoreboard players set @s weapon1_max_range_m 30
