# Permet d'acheter la weapon1 de niveau 8 pour le guerrier
...
advancement grant @s only warrior:weapon1lvl8_w
# Donne le haut fait "Hache de bataille VIII" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:carrot_on_a_stick",count:1,components:{"minecraft:custom_name":"Hache de bataille IX - 90 PH","custom_model_data":{strings:["battle_axe"]}}},ItemDropChance:1.0f}
