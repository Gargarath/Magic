# Permet d'acheter la weapon1 de niveau 9 pour le guerrier
...
advancement grant @s only warrior:weapon1lvl9_w
# Donne le haut fait "Hache de bataille IX" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:carrot_on_a_stick",count:1,components:{"minecraft:custom_name":"Hache de bataille X - 100 PH","custom_model_data":{strings:["battle_axe"]}}},ItemDropChance:1.0f}
