# Permet d'acheter la weapon1 de niveau 5 pour l'archer
...
advancement grant @s only archer:weapon1lvl5_a
# Donne le haut fait "Arc de combat V" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:bow",count:1,components:{"minecraft:custom_name":"Arc de combat VI - 60 PH","minecraft:enchantment_glint_override":true}},ItemDropChance:1.0f}
