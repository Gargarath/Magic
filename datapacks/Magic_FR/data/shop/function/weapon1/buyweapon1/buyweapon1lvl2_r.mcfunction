# Permet d'acheter la weapon1 de niveau 2 pour l'assassin
...
advancement grant @s only rogue:weapon1lvl2_r
# Donne le haut fait "Dague sournoise II" à @s

execute as @e[tag=weapon1,distance=..10,limit=1] run data merge entity @e[tag=weapon1,distance=..10,limit=1] {ItemRotation:0b,Item:{id:"minecraft:carrot_on_a_stick",count:1,components:{"minecraft:custom_name":"Dague sournoise III - 30 PH","custom_model_data":{strings:["dagger"]}}},ItemDropChance:1.0f}
