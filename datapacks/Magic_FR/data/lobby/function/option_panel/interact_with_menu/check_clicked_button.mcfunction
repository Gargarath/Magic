...
execute if entity @s[tag=optn_menu_lobby_operator_access_op] on target if score @s operator matches 2 run function lobby:option_panel/pages/lobby/operator_access/warning_message_op_are_admin with entity @s EnderItems[0].components.minecraft:custom_data
