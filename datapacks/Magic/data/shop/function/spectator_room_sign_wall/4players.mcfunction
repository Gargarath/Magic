# appelée par shop:shop_is_ready lorsqu'il y a 4 joueurs dans la partie, permet alors d'afficher les panneaux pour les observer
...
$setblock -1 -48 61 oak_wall_sign[facing=north]{front_text:{messages:[{"text":"$(tr_shop_spec)","color":"white","bold":true,"click_event":{"action":"run_command","command":"trigger spec_player3 add 1"}},"",{"nbt":"by_player.3.name","storage":"stats:leaderboards","interpret":true},""]},is_waxed:1b} replace
# place le panneau pour observer le joueur 3

$setblock -2 -48 61 oak_wall_sign[facing=north]{front_text:{messages:[{"text":"$(tr_shop_spec)","color":"white","bold":true,"click_event":{"action":"run_command","command":"trigger spec_player4 add 1"}},"",{"nbt":"by_player.4.name","storage":"stats:leaderboards","interpret":true},""]},is_waxed:1b} replace
