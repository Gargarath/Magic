...
bossbar set secondary_objective:state_blue color red
bossbar set secondary_objective:state_red color red
scoreboard players set state point_capture 2
# passe au state 2 (= les rouges contrôlent le point)

execute as @a run function gamemode:ctf/secondary_objectives/messages/red_controls with entity @s EnderItems[0].components.minecraft:custom_data
...
execute store result bossbar secondary_objective:state_red max run scoreboard players get max_red point_capture
execute store result bossbar secondary_objective:state_blue max run scoreboard players get max_red point_capture
