## called by rotate_player9
## extand the hook if possible

execute unless score @a[scores={Player=9},limit=1] hook_max_range_w matches 1 at @s positioned ~ ~0.5 ~ unless block ~ ~-45 ~ #spawn_protectors unless entity @a[tag=!hook9_immune,distance=..1] run function spells:spellsystem/spell3/spell3_w/hook/extand_hook/spawn_hook/player9
# If there is no player in front call the function that extand the hook

execute if score @a[scores={Player=9},limit=1] hook_max_range_w matches 1 as @a[scores={Player=9}] run function spells:spellsystem/spell3/spell3_w/hook/retract_hook/start_delete/player9
