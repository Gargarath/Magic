...
execute unless score @a[scores={in_trap_number=11},limit=1] health <= $6 numbers run scoreboard players add @s stat_spell_dmg 60
# si la cible a suffisament de pv ajoute les stats de dégats du sort au lanceur
execute as @a[scores={in_trap_number=11},limit=1] if score @s health <= $6 numbers run function main:stats/multiply_health_by_10
execute if score @a[scores={in_trap_number=11},limit=1] health <= $6 numbers run scoreboard players operation @s stat_spell_dmg += @a[scores={in_trap_number=11},limit=1] health_x10
...
bossbar set trapped_someone:player11 max 80
bossbar set trapped_someone:player11 value 80
